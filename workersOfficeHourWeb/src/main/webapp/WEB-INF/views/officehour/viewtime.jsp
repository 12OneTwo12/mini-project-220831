			...
			<table border="1" width="500" height="300" align="center">
				...
				<tr align="center" bgcolor="grey">
					...
					<c:forEach var="item" items="${list}" >
						<tr>
							<td>${item.workStart.monthValue}-${item.workStart.dayOfMonth}</td>
							<td>${item.workStart.year}년 ${item.workStart.monthValue}월 ${item.workStart.dayOfMonth}일 ${item.workStart.hour}시 ${item.workStart.minute}분</td>
							<td>${item.workEnd.year}년 ${item.workEnd.monthValue}월 ${item.workEnd.dayOfMonth}일 ${item.workEnd.hour}시 ${item.workEnd.minute}분</td>
							<td>${item.offday}</td>
						</tr>
					</c:forEach>
			</table>
